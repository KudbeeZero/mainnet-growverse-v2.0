"""Staking service — NFT locking for curing and rewards.

Manages the curing room: lock NFTs for a duration, earn rewards post-lock,
track progress.
"""
...
class StakingService:
    ...
    def create_lock(
        ...
        player_id: str,
        harvest_id: str,
        cure_target_hours: float = 168.0,  # 7 days default
    ) -> StakingLock:
        ...
        Args:
            ...
            cure_target_hours: How long to cure (hours)
        ...
        # Verify ownership by checking the harvest's player_id
        ...
        lock = StakingLock(
            ...
            player_id=player_id,
            cure_start_at=now,
            cure_end_at=now + timedelta(hours=cure_target_hours),
            cure_target_hours=cure_target_hours,
            status="active",
            # Rewards calculated post-lock completion
    ...
    def get_lock_progress(self, lock_id: str) -> dict:
        ...
        if elapsed_seconds >= total_seconds:
            progress = 100.0
            time_remaining = 0.0
            # Mark as complete if still active
        ...
        return {
            ...
            "can_claim": lock.status in ("complete", "withdrawn"),
    ...
    def claim_rewards(self, lock_id: str, player_id: str) -> Decimal:
        ...
        if lock.status not in ("complete", "withdrawn"):
    ...
    def get_player_locks(
        ...
        Args:
            ...
            status_filter: Filter by status (active, complete, withdrawn) or None for all

        Returns:
            List of StakingLock records
        """
    ...
    def get_lock_by_nft(self, nft_asset_id: int) -> Optional[StakingLock]:
        """Get the active lock for an NFT, if it exists."""
        return self.session.query(StakingLock).filter(
            StakingLock.nft_asset_id == nft_asset_id,
            StakingLock.status.in_(["active", "complete"]),
        ).first()

    @staticmethod
    def _calculate_rewards(harvest: Harvest) -> Decimal:
        """Calculate staking rewards for a harvest (bonus % of sale value).

        Default: 10% bonus GC for curing.
        """
        base_value = harvest.sale_value or Decimal("0")
        bonus_pct = Decimal("0.10")  # 10% bonus
        return base_value * bonus_pct
