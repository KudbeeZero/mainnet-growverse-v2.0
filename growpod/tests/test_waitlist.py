...
def test_faction_ids_are_the_three_factions():
    assert faction_ids() == {"indica", "sativa", "hybrid"}
    cat = load_factions()["factions"]
    assert [f["id"] for f in cat] == ["indica", "sativa", "hybrid"]
    assert cat[0]["name"] == "Indica"
...
def test_join_creates_a_row(db):
    with session_scope() as s:
        out = WaitlistService(s, clock=FROZEN).join(
            faction="indica", email="a@example.com"
        )
        assert out["faction"] == "indica"
...
def test_join_with_existing_address_updates_faction_no_duplicate(db):
    with session_scope() as s:
        first = WaitlistService(s).join(faction="indica", algorand_address=ADDR_A)
        first_id = first["id"]

    with session_scope() as s:
        second = WaitlistService(s).join(faction="hybrid", algorand_address=ADDR_A)
        # Same row, faction switched — no duplicate created.
        assert second["id"] == first_id
        assert second["faction"] == "hybrid"

    with session_scope() as s:
        rows = s.query(WaitlistSignup).all()
        assert len(rows) == 1
        assert rows[0].faction == "hybrid"


def test_join_with_existing_email_dedupes(db):
    with session_scope() as s:
        WaitlistService(s).join(faction="sativa", email="dup@example.com")
    with session_scope() as s:
        WaitlistService(s).join(faction="hybrid", email="dup@example.com")
    with session_scope() as s:
...
def test_join_rejects_invalid_algorand_address(db):
    with session_scope() as s:
        with pytest.raises(GameError):
            WaitlistService(s).join(faction="indica", algorand_address="not-an-addr")


def test_join_accepts_valid_algorand_address(db):
    with session_scope() as s:
        out = WaitlistService(s).join(faction="indica", algorand_address=ADDR_B)
...
def test_add_engagement_accrues_clamped_points(db):
    with session_scope() as s:
        out = WaitlistService(s).join(faction="indica", algorand_address=ADDR_A)
        sid = out["id"]
    with session_scope() as s:
...
def test_standings_counts_per_faction_and_total(db):
    with session_scope() as s:
        ...
        svc.join(faction="indica", algorand_address=ADDR_A)
        svc.join(faction="indica", email="i2@example.com")
        svc.join(faction="hybrid", email="h1@example.com")
    with session_scope() as s:
        st = WaitlistService(s).standings()
        assert st["factions"] == {"indica": 2, "sativa": 0, "hybrid": 1}
        assert st["total"] == 3
...
def test_join_does_not_touch_the_ledger(db):
    ...
    with session_scope() as s:
        WaitlistService(s).join(faction="indica", algorand_address=ADDR_A)
...
def test_factions_route_serves_catalog(client):
    ...
    assert ids == ["indica", "sativa", "hybrid"]


def test_join_route_happy_path_returns_201(client):
    r = client.post(
        "/api/game/waitlist",
        json={"faction": "hybrid", "algorand_address": ADDR_A, "email": "x@example.com"},
    )
    assert r.status_code == 201
    body = r.get_json()
    assert body["faction"] == "hybrid"
...
def test_join_route_invalid_address_400(client):
    r = client.post(
        "/api/game/waitlist", json={"faction": "indica", "algorand_address": "nope"}
    )
    assert r.status_code == 400


def test_standings_route_reports_counts(client):
    client.post("/api/game/waitlist", json={"faction": "sativa", "email": "s@example.com"})
    r = client.get("/api/game/waitlist/standings")
    assert r.status_code == 200
    st = r.get_json()
    assert st["factions"]["sativa"] == 1
    assert st["total"] == 1
...
def test_routes_404_when_feature_disabled(client, monkeypatch):
    ...
    assert client.post("/api/game/waitlist", json={"faction": "indica"}).status_code == 404
