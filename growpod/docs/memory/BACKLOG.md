...
2. 🔨 **Chain settlement verification (RISK #7)** — reconciliation job now built + tested (`## 🟠 Medium` Sprint 4 entry); remaining: deposit txid verify, replay protection → `## 🔴 Immediate`
3. ⬜ **Security follow-ups PR #104** — deposit redesign, CSP nonce, CORS allowlist, player key off localStorage, CODEOWNERS → `## 🏛️ HERMES`
4. ✅ **5 of 12 feature flags gate nothing** — wired (2026-07-03): `ftue_tutorial`/`grow_chamber`/`master_grower_advisor`/`breeding_lab`/`daily_stipend` now each decorate their real route → `### 🕵️ Dormant investments`
5. ✅ **Web gating never reads `GET /api/game/flags`** — re-pointed (2026-07-03): `web/src/lib/features.ts` now has `useFeatureFlags()` reading the live endpoint → `### 🕵️ Dormant investments`
...
24. 🔨 **Sprint 4: real TestNet + IPFS** — wiring/docs/reconciliation job/gated e2e test done; remaining needs owner: fund treasury, `reset_asa`, wire `ASA_ID` → `## 🟠 Medium`
...
- 🎮 ✅ **GameShell HUD redesign — swipe/dock edge-overlay panels + real landscape lock
  (2026-07-03, owner mockups `docs/memory/design/mockups/growverse-{mobile,desktop,
  landscape-mobile}-hud-concept.png`)** — restructures the chamber's navigation from three
  bottom-sheet tabs (ARCADE/CLIMATE/TIME) into the mockups' "one hero, everything else slides
  in/out" shell. New `web/src/components/shell/`: `GameShell.tsx` (owns left/right panel
  open/compact state + a persistent bottom dock; hero never resizes), `EdgePanel.tsx` (slim
  always-visible edge tab → glassmorphic overlay; content unmounts on close, not just
  translated off-screen, so a compacted panel is genuinely absent from the DOM/a11y tree),
  `OrientationGate.tsx` + `useOrientationLock.ts` (REAL landscape lock — Screen Orientation API
  + `matchMedia`, gated to touch+small-max-dimension devices so a portrait desktop monitor is
  never blocked — not a CSS-reflow hack), `useIsTouchDevice.ts` (branches swipe-gesture vs
  hover/click by `pointer: coarse`, not viewport width). Left panel (`ActionsPanel.tsx`) and
  right panel (`InsightsPanel.tsx`) are the SAME components on mobile and desktop — the third
  mockup's "Desktop-Matched Controls" callout — wired to real data/mutations only: the six care
  actions via the existing `useCareActions`/`careAvailability`, Plant Insights via the existing
  trichome/health derivation, Environment via VPD (`plant.metrics.vpd_kpa`)/Airflow (the fan
  setpoint)/Light Intensity (pod env)/Pressure (`pest_level`/`disease_level`) plus the existing
  `EnvironmentRail` for the real editable setpoints, Missions via `useContracts` (this codebase
  has no separate mission entity — contracts are the honest real analog), Inventory via
  `useConsumables`, Progress via `useLevel` + the existing `PlantProgressStrip`. Real existing
  mechanics that don't map onto the mockups' 5 sections (Arcade quick-boosts + paid growth-boost,
  the growth-day preview scrubber) were kept, not dropped, as extra collapsible sections — noted
  as a deliberate scope addition beyond the literal mockup. Mobile swipe-from-edge uses plain
  PointerEvents (no gesture-library dep); desktop hover-or-click opens a panel, closes via the ✕/
  tab-toggle/auto-compact-after-action only (no close-on-mouse-leave timer — the mockups never
  call for one and it just fights reaching a control). Desktop resize is a 3-preset (S/M/L)
  cycle persisted to `localStorage`, not free drag-resize — noted simplification. `care-loop-
  shot.spec.ts`'s chamber assertions updated for the new structure (bottom dock always visible;
  Insights/Boosts present only once the panel is opened, not permanently gone); new
  `hud-shell-shot.spec.ts` proves the portrait rotate-gate, a real touch swipe opening Actions
...
- 🟠 ✅ **5 of 12 feature flags gate nothing (closed 2026-07-03)** — `ftue_tutorial` now guards
  `/ftue/status`, `/ftue/coaching/<step>`, `/ftue/advance`; `grow_chamber` guards
  `/plants/<id>/state` (the whole-plant live-state read the chamber view uses); `master_grower_advisor`
  guards `/plants/<id>/advisor`; `breeding_lab` guards `/breed`; `daily_stipend` guards `/daily`. All
  seven routes use the same `@require_feature(<flag>)` outer-decorator pattern as the existing
  gated surfaces (404 via `FeatureDisabledError`, fires before `@require_player`). New coverage in
  `tests/test_game_api_gated_features.py` (flag OFF → 404, not 500) alongside existing ON-path tests
  (`test_ftue_routes.py`, `test_advisor_routes.py`, `test_game_api_errors.py`, `test_profile_routes.py`).
  Wiring itself was safe to land directly per the launch-readiness charter; flipping any flag's
  *default* off in prod is still a separate owner call.
- 🟠 ✅ **Web gating never reads `GET /api/game/flags` (closed 2026-07-03)** — `web/src/lib/features.ts`
  gained `useFeatureFlags()` (backed by `useBackendFlags()` in `hooks/queries.ts`, hitting the
  already-existing `GET /api/game/flags`), merging the backend's live map over the env-driven
  `FEATURES` fallback via the pure, unit-tested `mergeBackendFlags`. `RequireFeature`, `navLinks.ts`
  (new `useNavLinks()`, used by `NavBar`/`MobileTabBar`), `market/page.tsx`, `profile/page.tsx`, and
  `university/learner/page.tsx` all now read the live hook instead of the static env-only map — a
  backend kill switch (`FEATURE_MARKETPLACE=false` or a `balance.yaml` flag flip) now actually hides
  the gated nav entry/route in the web client, not just the API.
- 🟠 ⬜ **Chamber's AIRFLOW (fan) slider is cosmetic-only, never persisted** — found by the new
  UI Audit Loop (`docs/memory/UI_AUDIT_MAP.md`). `web/src/app/dashboard/plants/[plantId]/chamber/
  page.tsx`'s `onSlide()` explicitly skips the commit call for the `"fan"` key
  (`if (key !== "fan") scheduleCommit(next)`), and `InsightsPanel.tsx`'s prop comment says so
  outright. This predates real fan mechanics; now that fans have a genuine simulation effect
  (condition-scaled humidity reduction, `simulation/engine.py::_equipped_condition_effects`, this
  session), a player-facing airflow knob that does nothing is actively misleading, not just inert.
  **Fix:** either wire it to the real `POST /players/<id>/pods/<pod_id>/equip-fan` flow or remove
  the control until it's wired.
- 🟡 ⬜ **"Enter Cup" on the profile harvest card is a never-passed optional prop** — found by
  the UI Audit Loop. `web/src/components/harvest/HarvestsPanel.tsx`'s `HarvestCard` only renders
  the button when `onEnterCup` is supplied; `app/profile/page.tsx` renders `<HarvestsPanel />`
  with no props, so the button silently never appears — not broken, just never finished. Cup
  entry today only works via `/cup`'s own "Enter a harvest" modal. **Fix:** wire `onEnterCup`
  from `/profile` to `api.cup.enter`, or drop the dead prop.
...
- 🟠 🔨 **Sprint 4: real TestNet + IPFS** (2026-07-03) — audited `chain/factory.py` + `config.py`:
  the mock→real switch was already a pure config change (`ALGO_TREASURY_MNEMONIC`/`ASA_ID`/
  `USE_MOCK_CHAIN`/etc., no hardcoded values); documented it end-to-end in
  `docs/TESTNET_SETUP.md` (env vars, how to create + fund a TestNet account via the public
  dispenser, what going live requires). `services/ipfs.py` now defaults to a deterministic
  **offline** mock hash (no implicit `localhost:5001` network attempt) when neither `PINATA_JWT`
  nor `IPFS_API_URL` is set — the real Pinata path (`requests.post` to `pinFileToIPFS`, JWT from
  env) was already complete and is unchanged; going live is still just setting `PINATA_JWT`.
  Fixed a latent crash in `services/nft_mint.py._build_harvest_metadata` (`harvest.strain`/
  `harvest.player` don't exist — `Harvest` has no ORM relationship for those FKs — so any mint
  attempt would have thrown `AttributeError`; nothing had ever exercised that path). Wired the
  metadata hash into `create_asset_tx` (was `None`, TODO). Built
  `services/reconciliation.py::reconcile_nft_assets` — the DB↔chain drift job — + 5 tests against
  `MockChainProvider` (a manufactured "DB says minted, chain never confirmed" case, a
  chain-side-destroyed case, and an invariant check on ASA `total`). Added
  `tests/test_live_testnet_e2e.py`: a mock-based mint→list→buy→stake→claim loop (always runs,
  found + fixed the metadata bug above) plus a `pytest.mark.skipif`-gated live-TestNet twin.
  *Remaining (⬜, needs the owner — an agent cannot create/fund a real wallet): generate +
  fund a TestNet treasury account via the dispenser (`docs/TESTNET_SETUP.md` §3), set
  `ALGO_TREASURY_MNEMONIC`, then run `reset_asa`/set `ASA_ID`; fund a second TestNet account
  (`ALGO_TEST_BUYER_MNEMONIC`) to actually run the live e2e test; deposit txid replay
  protection (`onchain_txid` uniqueness) is still open, tracked under RISK #7 above.*
