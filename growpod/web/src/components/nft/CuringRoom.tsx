...
export function CuringRoom() {
  ...
  const locks = useQuery({
    ...
    queryFn: () => (playerId ? api.stakes.getLocks(playerId) : Promise.resolve([])),
    enabled: !!playerId,
    refetchInterval: 10000, // Update every 10s to show live progress
  });
  ...
  const renderLock = (lock: StakingLock) => {
    ...
    return (
      ...
      >
        ...
        {/* Progress ring (for active locks) */}
        {lock.status === "active" && (
    ...
    <div className="space-y-4">
      ...
      <Card className="border-purple-700/40 bg-purple-900/20">
        ...
        <div className="space-y-1 text-sm text-gray-400">
          <p>1. Lock an NFT harvest in the curing room</p>
          <p>2. Wait for the cure duration (typically 7–14 days)</p>
          <p>3. Earn a quality-based bonus % on top of sale value</p>
